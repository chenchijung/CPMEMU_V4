...
#ifndef Z80_YAZE110 // Z80_YAZE110 is not defined -> Z80_YAZEAG_2_51_3
char *perl_params =
    "combine=0,"
    "optab=0,"
    "cb_inline=1,"
...
//#define DEBUG //#dchen optimize for speed, reduce per cycle execution checking
/* Z80 registers */
...
void output(BYTE port)
{
	if (port != 0xf0) return;
	else exit_loop = 1; /* stop simulation */
}
...
simz80(FASTREG PC)
{
	...
	case 0x76:			/* HALT */
		...
		halt = 1;		// #dchen mark halt flag
		exit_loop = 1;
		//stopsim = 1;	// #dchen quit from CPU execution loop
		SAVE_STATE();
		return PC&0xffff;
	case 0x77:			/* LD (HL),A */
		PutBYTE(HL, hreg(AF));
...
#if 0 // #dchen per instruction operation: optimized (remove for faster)
	//printf("PC=%04X AF=%04X BC=%04X DE=%04X HL=%04X IX=%04X IY=%04X\n",PC, (AF&0xFFFF), (BC&0xFFFF), (DE&0xFFFF), (HL&0xFFFF), ix, iy);
    if (Z80DebugFlag)
    {
    	fprintf(Z80Trace, "PC=%04X AF=%04X BC=%04X DE=%04X HL=%04X IX=%04X IY=%04X\n",PC, (AF&0xFFFF), (BC&0xFFFF), (DE&0xFFFF), (HL&0xFFFF), ix, iy);
	}
#endif
    }
exit_lp:
...
#endif // Z80_YAZE110 is not defined